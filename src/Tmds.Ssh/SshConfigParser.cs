...
sealed class SshConfigParser
{
    ...
    {
        foreach (string line in ReadConfigLines(filePath))
        {
            if (!TryGetLineAsSpan(line, out ReadOnlySpan<char> remainder))
            {
                continue;
            }

            if (!TryGetNextToken(ref remainder, out ReadOnlySpan<char> keyword))
            {
                continue;
            }
            ...
            if (keyword.Equals("host", StringComparison.OrdinalIgnoreCase))
            {
                ...
                {
                    if (pattern.ContainsAny('*', '?') || pattern.StartsWith("!"))
                    {
                        continue;
                    }

                    hosts.Add(pattern.ToString());
                }
            }
            else if (keyword.Equals("include", StringComparison.OrdinalIgnoreCase))
            {
                while (TryGetNextToken(ref remainder, out ReadOnlySpan<char> pathPattern))
                {
                    ...
                    foreach (var includedFile in Glob(path))
                    {
                        CollectHosts(includedFile, includeBasePath, hosts);
                    }
                }
            }
        }
    ...
    internal static ValueTask<SshConfigParser> DetermineConfigForHost(string? userName, string host, int? port, IReadOnlyDictionary<SshConfigOption, SshConfigOptionValue>? options, IReadOnlyList<string> configFiles, CancellationToken cancellationToken)
    {
        SshConfigParser config = new SshConfigParser()
        {
            UserName = userName,
            Port = port
        };
    ...
    private static void ConfigureFromOptions(SshConfigParser config, IReadOnlyDictionary<SshConfigOption, SshConfigOptionValue> options)
    {
    ...
    private static void ConfigureFromConfigFile(SshConfigParser config, string host, string originalhost, string filename, string includeBasePath, ref bool performSecondPass, bool isSecondPass)
    {
        Span<char> keywordBuffer = stackalloc char[MaxKeywordLength];
        bool isMatch = true;
    ...
    private static void HandleMatchedKeyword(SshConfigParser config, ReadOnlySpan<char> keyword, ReadOnlySpan<char> keywordValue)
    {
    ...
    private static bool IsMatchMatch(SshConfigParser config, string host, string originalhost, ReadOnlySpan<char> matchPattern, ref bool performSecondPass, bool isFinal)
    {
    ...
    private static bool TryGetLineAsSpan(string line, out ReadOnlySpan<char> remainder)
    {
        remainder = line.AsSpan().Trim(WhiteSpace);
